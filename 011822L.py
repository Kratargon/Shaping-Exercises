...
class Game:
    def __init__(self, playerCount = 2, size = 8):
        ...
        self.playerBoards = boardArray

    def getBoard(self, player = 0):
        if player < len(self.playerBoards):
            return self.playerBoards[player]
        print("This player doesn't exist!")

def main(runWith = "terminal"):
    if runWith == "terminal":
        activeGame = terminalStart()
        terminalSetup(activeGame)

def terminalStart():
    print("Welcome to Battleship!")
    changeBool = input("Would you like to change the settings from default?")
    if changeBool == "y":
        playerCount = int(input("Player Count?"))
        size = int(input("Size?"))
        activeGame = Game(playerCount, size)
    else:
        activeGame = Game()
    return activeGame

def terminalSetup(activeGame):
    print("Set up your board!")
    manualBool = input("Would you like to choose the locations for your ships manually?")
    if manualBool == "y":
        for i in activeGame
        #TODO add set up ship placement